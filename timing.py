...
for util in [50, 60, 70, 80, 90]:
    ...
    # UUnifast by KHCHEN
    ###
    # task_sets_generator = gm.gen_tasksets(5, args.r, 1, 100, args.u, rounded=True)
    task_sets_generator = gm.gen_tasksets_pred(no_tasks, no_task_sets, 1, 2000, float(util)/100.0, [1,2,5,10,20,50,100,200,500,1000])
    ...
    ce_chains = gm.gen_ce_chains(task_sets)
    ...
    print("test: davare")
    analyzer.davare(ce_chains) # e2e_latency
    print("test: reaction CASES19")
    analyzer.reaction_sporadic(ce_chains) #jj_react
    print("test: age CASES19")
    analyzer.age_sporadic(ce_chains) #jj_age
    ...
    for task_set in task_sets:
        ...
        chain = random.choice(ce_chains[i])
        # OUR RESULTS
